  ...
  : m_solSys(astro::SolarSystem::EARTH), m_params(params) 
{
  ...
  if (m_TimingNoiseModel == 1) 
      ...
      if (TNOISELOG==1)
	{
	  ...
	  TimingNoiseLogFile << "tMET\t\P\tf0\tf2\tDeltat\tDelta8\tdt"<<std::endl;
  ...
  if (m_BinaryFlag == 1)
	...
	PulsarLog << "**   No Post Newtonian Parameterization " << std::endl; 
      ...
      if ((m_BinaryFlag ==1) && (BINDEMODLOG))
 	{
      ...
      if (BARYCORRLOG)
	{
	  std::ofstream BaryCorrLogFile((m_PSRname + "BaryCorr.log").c_str(),std::ios::out);
	  BaryCorrLogFile << "Pulsar" << m_PSRname << " : Barycentric corrections log generated by PulsarSpectrum" << std::endl;
...
{  
  ...
  double Delta8,Deltat=0;
  double intPart=0.; //Integer part
  double nextTimeTildeDemodulatedNoNoise =0.;

  if (m_TimingNoiseModel ==1)
    {
      //computing the f2=0 (pure linear model)
      m_f2 = 0.;
      double initTurnsNoNoise = getTurns(timeTildeDemodulated);
      double tStartNoNoise = modf(initTurnsNoNoise,&intPart)*m_period;
      double inteNoNoise = m_spectrum->interval(tStartNoNoise,m_enphmin); //deltaT (in system where Pdot = 0
      double inteTurnsNoNoise = inteNoNoise/m_period; // conversion to # of turns
      double totTurnsNoNoise = initTurnsNoNoise + inteTurnsNoNoise + m_phi0; //Total turns at the nextTimeTilde
      nextTimeTildeDemodulatedNoNoise = retrieveNextTimeTilde(timeTildeDemodulated, totTurnsNoNoise, (ephemCorrTol/m_period));

      Delta8 = 6.6 + 0.6*log10(m_pdot) + 1.0*(2.*m_PSpectrumRandom->Uniform()-1);
      double dt3 = pow((timeTildeDemodulated-(StartMissionDateMJD)*SecsOneDay),3.);
      Deltat = Delta8+3.0*log10((timeTildeDemodulated-(StartMissionDateMJD)*SecsOneDay)*1e-8);
     
     double Sign = m_PSpectrumRandom->Uniform();
     if (Sign > 0.5)
       m_f2 = ((m_f0*6.*std::pow(10.,Deltat))/dt3);
     else
       m_f2 = ((m_f0*6.*std::pow(10.,Deltat))/dt3);
  ...
  if ((m_TimingNoiseModel != 0) && (TNOISELOG==1))
    {
      ...
      TimingNoiseLogFile << std::setprecision(30) << nextTimeTildeDemodulated
			 << "\t" << m_f0 << "\t" << m_f2 << "\t" << Delta8
			 << "\t" << Deltat << "\t" << nextTimeTildeDemodulated << std::endl;
      std::cout << std::setprecision(30) << "Timing Noise: Delta8="<< Delta8 << " Delta=" << Deltat << " f2=" << m_f2 
		<< " dt=" << nextTimeTildeDemodulated - nextTimeTildeDemodulatedNoNoise << std::endl;
    }




  //std::cout << "TT" << nextTimeTildeDemodulated << std::endl;
  ...
  if ((LogCorrFlag == 1) && (ttInput-StartMissionDateMJD*SecsOneDay > 0.))
    {
	  ...
	  BaryCorrLogFile << std::setprecision(15)
			  << timeMET << "\t" << tdb_min_tt << "\t" << GLASTPosGeom << "\t" 
			  << EarthPosGeom << "\t" << GeomCorr << "\t" << ShapiroCorr << std::endl;
	  BaryCorrLogFile.close();
    }
