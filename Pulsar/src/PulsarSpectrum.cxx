...
int PulsarSpectrum::saveBinDbTxtFile()
{
  ...
  if (Flag == 0)
    {
      ...
      DbBinOutputFile << "PSRNAME PB PBDOT A1 XDOT ECC ECCDOT OM OMDOT GAMMA OBSERVER_CODE SOLAR_SYSTEM_EPHEMERIS SHAPIRO_R SHAPIRO_S";
  ...
  DbBinOutputFile << std::setprecision(3) << m_Porb << " " << m_Porb_dot << " ";   // Orbital period and derivative
  DbBinOutputFile << std::setprecision(3) << m_asini << " " << m_xdot << " ";      // Projected semi-mayor axis and derivative
  DbBinOutputFile << std::setprecision(3) << m_ecc << " " << m_ecc_dot << " ";     // Eccentricity and derivative
  DbBinOutputFile << std::setprecision(3) << m_omega << " " << m_omega_dot << " "; // Long. Periastron and derivative
  DbBinOutputFile << std::setprecision(3) << m_gamma << " ";                       // PPN gamma paramater  
  DbBinOutputFile << "MR \"JPL DE200\" ";                                          // Observer code and ephemerides
  DbBinOutputFile << std::setprecision(3) << m_shapiro_r << " " << m_shapiro_s << std::endl; //Shapiro Parameters
