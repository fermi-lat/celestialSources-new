...
// Constructor
GbmGrb::GbmGrb(HepRandomEngine *engine, const std::string &prefix, const std::vector<double> &specnorm, const std::string &dir)
    : GRB()
{
    m_specnorm = specnorm;
...
//      epeak                       :   
//      m_cjoin                     :   epeak ** (alpha-beta)
//      m_norm                      :
//      s_emax                      :   static variable
//      grbcst::ethresGBM           :   constant
...
long GbmGrb::calcNphoton(HepRandomEngine *engine)
{
    ...
    std::cout << "duration: " << duration << " alpha: " << m_globalData->alpha() << " beta: " << m_globalData->beta() <<
        " epeak: " << m_globalData->epeak() << std::endl;
    
    double m_cjoin = pow(epeak, (alpha-beta));
    double m_norm  = m_cjoin * (pow(epeak, gamma) - pow(grbcst::ethresGBM, gamma))/gamma + 
        (pow(s_emax, delta) - pow(epeak, delta))/delta;
    ...
    std::cout << "nphoton: " << long(factBrkplaw * m_specnorm[isim] * duration * gbmGeoArea/denom) << std::endl;
    //exit(0);
...
void GbmGrb::makeEnergies(HepRandomEngine *engine)
{
    ...
    std::transform(rands.begin(), rands.end(), rands.begin(), GRBobsUtilities::randGen(engine));
    
    std::vector<long> dumlo(grbcst::nbsim);
    std::vector<long> dumhi(grbcst::nbsim);
    
    // Fraction of photons below the epeak break
    double fracbreak = m_cjoin * (pow(epeak, gamma) - pow(grbcst::ethresGBM, gamma)) / (gamma * m_norm);
    ...
    for (iphot=0; iphot<nlo; ++iphot)
    {
        ...
        m_photonlist[iphot].setEnergy(exp(log(m_norm*temp*gamma/m_cjoin + pow(grbcst::ethresGBM, gamma)) / gamma));
    }
    
    for (iphot=0; iphot<nhi; ++iphot)
    {
        temp = rands[dumhi[iphot]];
        m_photonlist[iphot].setEnergy(exp(log(m_norm*temp - m_cjoin*(pow(epeak, gamma) - pow(grbcst::ethresGBM, gamma))/gamma)));
    }
}
