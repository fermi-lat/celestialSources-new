...
void galacticTest(FluxMgr* fm, std::string sourceName, double SimulationTime)
{
  ...
  while (time<SimulationTime)
    {
      ...
      if(energy>0.1 && time<SimulationTime)
	{
	  i++;
	  if(i<100) std::cout << "time= "<<time<<" energy= " << energy << " (theta,phi)= " <<theta*180./M_PI<<", "<<phi*180./M_PI<<" l,b =  "<<l<<" "<<b<<" interval "<<interval<<std::endl;
...
int main(int argn, char * argc[]) {
  ...
  if ( argn >2 ) SimulationTime = ::atof(argc[2]);
  ...
  if(argn != 1){
    galacticTest(&fm,source_name,SimulationTime);
    return 0;
  }
  ...
  for(abc= allTheSources.begin() ; abc != allTheSources.end() ; abc++)
    {
      ...
      fm.test(out, (*abc), (int) SimulationTime);
