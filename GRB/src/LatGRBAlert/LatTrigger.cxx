...
LatTrigger::LatTrigger(const std::vector<PhotonInfo> &photonData, const long nbckoff, const long nGrb, const long nBck)
	:m_falseTrig(0),
	 m_nbckoff(nbckoff),
...
bool LatTrigger::isSignalSet::operator () (const PhotonInfo &x)
{
	return x.signal() == 1;
}
...
bool LatTrigger::isFalseTrigger(const long region)
{
...
double LatTrigger::firstTriggerTime(const long region, const std::vector<PhotonInfo> &photonData)
{
	if ((region >= m_nbckoff) && (m_jointLike[region] > m_jointLike[region-m_nbckoff]))
...
void LatTrigger::recordFalseTriggers(const std::vector<long> &vindex)
{
